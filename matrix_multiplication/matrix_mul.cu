...
int main() {
    ...
    gpuWarmup(); // Warm up GPU before measurements [cite: 12]

    std::cout << std::fixed << std::setprecision(3);
    std::cout << "Starting Matrix Multiplication Performance Comparison..." << std::endl;
    std::cout << "TILE_DIM for Tiled Kernel: " << TILE_DIM << std::endl;
    std::cout << "Number of runs for averaging: " << NUM_RUNS << std::endl << std::endl;
    ...
    for (int N : matrix_sizes) {
        ...
        if (matrix_elements == 0) { // Avoid division by zero for N=0 (though sizes start at 4x4)
            std::cerr << "Matrix size N must be greater than 0." << std::endl;
            continue;
        }
        ...
        dim3 threadsPerBlockNaive(TILE_DIM, TILE_DIM); // Can be different from TILE_DIM used for tiled, e.g. 16x16 or 32x32
        ...
        // Shared memory size passed via kernel execution configuration [cite: 6]
        unsigned int shared_mem_size_bytes = 2 * TILE_DIM * TILE_DIM * sizeof(float); 
        ...
        std::cout << std::left << std::setw(12) << n_str
...
bool verifyResults(const float* ref_C, const float* C_to_verify, int N_rows, int N_cols, const std::string& matrix_name) {
    ...
    for (long i = 0; i < static_cast<long>(N_rows) * N_cols; ++i) {
        ...
        // A more robust check for floating point comparison:
        // if (std::fabs(ref_C[i] - C_to_verify[i]) > EPSILON && std::fabs(ref_C[i] - C_to_verify[i]) / std::fabs(ref_C[i]) > EPSILON) { // Relative and absolute
            if (pass) { // Print header only once per matrix verification
            ...
            pass = false;
            // return false; // Optionally, uncomment to stop at first error for faster feedback during debugging
        }
    }
    return pass;
}
...
void gpuWarmup() {
    std::cout << "Performing GPU warmup..." << std::endl;
    float* dummy_d;
    size_t dummy_size = 1024 * sizeof(float);
    CUDA_CHECK(cudaMalloc(&dummy_d, dummy_size));
    
    float* dummy_h = new float[1024];
    for(int i=0; i<1024; ++i) dummy_h[i] = 0.f; // Initialize to avoid UMR

    CUDA_CHECK(cudaMemcpy(dummy_d, dummy_h, dummy_size, cudaMemcpyHostToDevice));
    
    // Simple kernel launch for warmup
    dim3 threadsPerBlock(256);
    dim3 numBlocks(4); // 1024 total threads
    // A dummy kernel could be defined here, or use an existing one with minimal data
    // For simplicity, we can just use a small naiveMatMulKernel if N is small or a dedicated tiny kernel.
    // Or just rely on the cudaMemcpy and the first actual kernel run.
    // The cudaDeviceSynchronize ensures operations are completed.
    
    CUDA_CHECK(cudaFree(dummy_d));
    delete[] dummy_h;
    CUDA_CHECK(cudaDeviceSynchronize()); // Ensure warmup operations are complete
    std::cout << "GPU warmup complete." << std::endl << std::endl;
}
...
void printDeviceProperties() {
    ...
    cudaDeviceProp prop;
    int device;
    CUDA_CHECK(cudaGetDevice(&device)); // Get current device
    CUDA_CHECK(cudaGetDeviceProperties(&prop, device));

    std::cout << "--- Hardware and Runtime Environment (Partial) ---" << std::endl; // [cite: 17]
    ...
    // Note: For the report, also include OS and nvcc compiler version manually [cite: 17]
}